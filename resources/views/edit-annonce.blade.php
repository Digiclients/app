        ...
        @hasanyrole('particulier|professionnel')
            ...
            <section class="px-4 px-xl-0 pt-5 mt-5 container-fluid row justify-content-center">
                <div class="col-lg-12 col-xl-11 col-xxl-9 py-5 bgLight rounded row px-3">
                    <div class="pb-4">
                        <h1 class="font30 float-start text-uppercase fontwbold primarycolor primaryfont ms-2">Modifier l'annonce
                        </h1>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            const selectedValues = {
                carburant: "{{ $attributesData['carburant']['id'] ?? '' }}",
                boite_vitesse: "{{ $attributesData['boite_vitesse']['id'] ?? '' }}",
                couleur: "{{ $attributesData['couleur']['id'] ?? '' }}",
                permis: "{{ $attributesData['permis']['id'] ?? '' }}",
                crit_air: "{{ $attributesData['crit_air']['id'] ?? '' }}",
                etat_du_vehicule: "{{ $attributesData['etat_du_vehicule']['id'] ?? '' }}",
                type_vehicule: "{{ $attributesData['type_vehicule']['id'] ?? '' }}",
                nombre_place: "{{ $attributesData['nombre_place']['id'] ?? '' }}",
                nombre_porte: "{{ $attributesData['nombre_porte']['id'] ?? '' }}"
                // Add other attributes if needed
            };
                    ...
                    Object.keys(data).forEach(slug => {
                        ...
                        if (select) {
                            ...
                            data[slug].forEach(option => {
                                ...
                                if (option.id == selectedValues[slug]) {
                                    optionElement.selected = true;
                                    console.log(option.id, selectedValues[slug])
                                }
                                select.appendChild(optionElement);
                            });
