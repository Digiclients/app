...
class ZipCodeSeeder extends Seeder
{
    ...
    public function run(): void
    {
        // Fetch distinct city and zipcode pairs from the LeboncoinData table
        $zipCodes = LeboncoinData::select('city', 'zipcode')
            ->whereNotNull('city')
            ->whereNotNull('zipcode')
            ->distinct()
            ->pluck('zipcode', 'city'); // 'city' as key, 'zipcode' as value

        foreach ($zipCodes as $cityName => $zipCode) {
            $city = City::where('city', $cityName)->first(); // Ensure column name matches

            if ($city) {
                ZipCode::updateOrCreate(
                    ['city_id' => $city->id, 'zip_code' => $zipCode], // Unique constraints
                    ['description' => $zipCode . ' description']
                );
            }
        }
